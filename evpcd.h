//[[ Single header version of evpcd. ]]
#pragma once

/*
NOTES TO SELF:
in the next revisions, re-design the ISA, a minimal set works for now.
You don't need to perfect it every time so, if you want backwards compatibility
Keep the current instructions, Just add, even if the opcodes are not organized, that doesn't
matter if you got an assembler.
*/
...
//reg 252 = RET
//reg 255 = HOST
//NOTE: this is merely convention for the ret and host regs, Not obligatory
...
void evpcd_isa_btset(){
    ...
    uint32_t val1 = SET_BIT(val2, val3);
...
void evpcd_isa_btclr(){
    ...
    uint32_t val1 = CLR_BIT(val2, val3);
...
void evpcd_isa_bttest(){
    ...
    uint32_t val1 = TEST_BIT(val2, val3);
    if(val1){
